...
SendingPort::SendingPort(): Port(),bcastflag_(0)
{
...
SendingPort::SendingPort(char *hostname, short port):Port(),bcastflag_(0)
{
  setHostname(hostname);
  setPort(port);
...
Packet* ReceivingPort::receivePacket()
{
  ...
  //tmpBuffer_ point shall not be moved,it will reused by the receiving port.
 ...
 */
LossyReceivingPort::LossyReceivingPort(float lossyratio): ReceivingPort(), loss_ratio_(lossyratio)
{
}
